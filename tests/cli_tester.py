...
async def main():
    ...
    session = await runner.session_service.create_session(
        app_name="app", user_id="cli_tester"
    )
    ...
    while True:
        try:
            ...
            session = await runner.session_service.get_session(
                session_id=session_id, app_name="app", user_id="cli_tester"
            )
            if session:
                state_dict = (
                ...
                print(
                    f"[Debug Memory] Profile: location={profile.get('location_name')}, land={profile.get('land_size_acres')}"
                )
