...
from app.core.schemas import GraphState, FarmerProfile, MarketOutput
...
async def test_market_node_multiple_crops(monkeypatch):
    ...
    ctx.run_node.side_effect = lambda node, node_input, **kwargs: MarketOutput(
        ...
        state="Tamil Nadu",
        prices=[
            {"state": "Tamil Nadu", "district": "Salem", "commodity": "Cotton", "modal_price": "6000"},
            {"state": "Tamil Nadu", "district": "Salem", "commodity": "Sorghum", "modal_price": "2500"}
        ],
    ...
    def side_effect(crop, state):
        if crop == "Cotton":
            return [{"state": state, "district": "Salem", "commodity": "Cotton", "modal_price": "6000"}]
        elif crop == "Sorghum":
            return [{"state": state, "district": "Salem", "commodity": "Sorghum", "modal_price": "2500"}]
        return []
