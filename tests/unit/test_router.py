...
def test_router_forwards_to_specialists():
    state = GraphState(
        active_agents=["weather_agent", "market_agent"], missing_info_questions=[]
    )
...
def test_router_returns_direct_response_when_missing_info():
    ...
    assert event.actions.route == ROUTE_DIRECT_RESPONSE
    assert "location" in event.output


def test_router_returns_direct_response_when_no_active_agents():
    state = GraphState(active_agents=[], missing_info_questions=[])
    ...
    assert (
        event.output
        == "I am the Kisan Agent. How can I assist you with your farming needs today?"
    )
