...
- **Model:** Gemini (`gemini-3.5-flash`) via `google-genai` SDK.
...
4. **Testing Policy:** Do NOT run the full test suite (`uv run pytest`) after every minor change, as it consumes API quota by making real calls to the LLM. You MUST NEVER run tests on your own without asking for explicit confirmation from the user first. Only run tests after major architectural changes, new feature additions, or before deployments, AND only after the user approves. Use `agents-cli dev` (the ADK playground) for interactive, manual verification of small tweaks.
